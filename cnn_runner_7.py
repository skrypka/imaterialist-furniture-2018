...
from misc import FurnitureDataset, preprocess, preprocess_with_augmentation, NB_CLASSES, normalize_torch
...
def get_model():
    ...
    model = models.se_resnet101_finetune(NB_CLASSES)
    if use_gpu:
        model.cuda()
    print('done')
    return model
...
def train():
    ...
    for epoch in range(20):
        ...
        if patience == 2:
            patience = 0
            model.load_state_dict(torch.load('se_resnet101.pth'))
            lr = lr / 10
            print(f'[+] set lr={lr}')
        if epoch == 0:
            lr = 0.001
        ...
        if log_loss < min_loss:
            torch.save(model.state_dict(), 'se_resnet101.pth')
